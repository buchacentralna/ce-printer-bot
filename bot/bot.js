...
if (!token) {
  console.error('CRITICAL: BOT_TOKEN is not set. Telegram bot will not start.');
} else {
  ...
  bot.catch((err, ctx) => {
    console.error(`❌ Помилка в обробнику для оновлення ${ctx.updateType}:`, err);
    ...
    try {
      ...
      ctx.reply('❌ Сталася помилка при обробці вашого запиту. Спробуйте ще раз або зверніться до адміністратора.', keyboard);
    } catch (e) {

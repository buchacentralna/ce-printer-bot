...
import { checkAccess, getUserStats, logPrintAction, getAdminList } from '../gsheets/index.js';
 ...
 */
function resetPrintSession(ctx) {
  if (ctx.session.currentFile) {
    if (ctx.session.currentFile.sourcePaths) {
      cleanupFiles(ctx.session.currentFile.sourcePaths);
    }
    cleanupFiles(ctx.session.currentFile.path);
  }
  if (ctx.session.multiImages) {
    cleanupFiles(ctx.session.multiImages.map(img => img.path));
  }
...
async function editWizardStep(ctx, text, keyboard, mediaBuffer = null) {
  ...
  const messageId = ctx.callbackQuery?.message?.message_id || ctx.session.lastWizardMsgId;

  if (!messageId) {
    if (mediaBuffer) {
      const res = await ctx.replyWithPhoto({ source: mediaBuffer }, { caption: text, ...keyboard });
  ...
  const isMedia = msg ? !!(msg.photo || msg.document) : (!!mediaBuffer || !!ctx.session.currentFile?.preview);
...
async function showWizardStepCPP(ctx) {
  ...
  const preview = await getLivePreview(ctx);
  await editWizardStep(ctx, 'Крок 2/4: Копій кожної сторінки на аркуші:', keyboard, preview);
}
...
async function showWizardStepPPS(ctx) {
  ...
  const buttons = filteredOptions.map(opt => [Markup.button.callback(opt.label, `wizard_pps_${opt.id}`)]);
  buttons.push([Markup.button.callback('⬅️ Назад', 'wizard_back_to_cpp')]);

  const keyboard = Markup.inlineKeyboard(buttons);
  const preview = await getLivePreview(ctx);
  await editWizardStep(ctx, 'Крок 3/4: Сторінок на аркуші (унікальних):', keyboard, preview);
}

async function showWizardStepCopies(ctx) {
  ctx.session.currentWizardStep = 'copies';
  const keyboard = Markup.inlineKeyboard([
    [Markup.button.callback('1', 'wizard_copies_1'), Markup.button.callback('2', 'wizard_copies_2')],
    [Markup.button.callback('5', 'wizard_copies_5'), Markup.button.callback('10', 'wizard_copies_10')],
    [Markup.button.callback('🔢 Інше', 'wizard_copies_other')],
    [Markup.button.callback('⬅️ Назад', 'wizard_back_to_layout')]
  ]);
  const preview = await getLivePreview(ctx);
  await editWizardStep(ctx, 'Крок 4/4: Загальна кількість тиражу (копій):', keyboard, preview);
}

async function renderCurrentWizardStep(ctx) {
  const step = ctx.session.currentWizardStep || 'color';
  switch (step) {
    case 'color': return showWizardStep1(ctx);
    case 'cpp': return showWizardStepCPP(ctx);
    case 'pps': return showWizardStepPPS(ctx);
    case 'copies': return showWizardStepCopies(ctx);
    case 'summary': return showFinalSummary(ctx);
    default: return showWizardStep1(ctx);
  }
}
...
export function registerHandlers(bot) {
  ...
  bot.use(async (ctx, next) => {
    if (ctx.session) {
      ...
      if (ctx.session.lastActivity && (now - ctx.session.lastActivity > hour) && ctx.session.currentFile) {
        resetPrintSession(ctx);
        await ctx.reply('Вибачте, час очікування сплив 😔', Markup.inlineKeyboard([
          [Markup.button.callback('🔄 Почати заново', 'type_selection_restart')]
        ]));
  ...
  bot.start(async (ctx) => {
    const chatId = ctx.chat.id;
    const isAllowed = await checkAccess(chatId);

    if (!isAllowed) {
      return ctx.reply('На жаль, у вас немає доступу до цього бота. Зверніться до адміністратора.');
    }
  ...
  bot.hears(['Служіння', 'Особисте'], async (ctx) => {
    ...
    await ctx.reply(
      ...
      Markup.inlineKeyboard([
        [Markup.button.callback('🖼️ Друкувати багато картинок', 'multi_image_start')]
      ])
    );
  });
  ...
  bot.on(['photo', 'document', 'sticker'], async (ctx) => {
    ...
    if (!ctx.session.multiImageMode && !ctx.session.currentFile) {
      ...
      try {
        if (ctx.session.lastMultiMsgId) {
          await ctx.telegram.editMessageText(ctx.chat.id, ctx.session.lastMultiMsgId, null, statusText);
    ...
    try {
      ...
      const isPhoto = !!ctx.message.photo || !!ctx.message.sticker ||
      ...
      if (isPhoto) {
        try {
          ...
          console.warn('Compression failed, using original buffer:', err.message);
        }
      }
      ...
      if (ctx.session.currentFile && !ctx.session.multiImageMode) {
        ...
        if (isPhoto && hasPrevImages) {
          ...
          // Перезібрати PDF
          const imageBuffers = ctx.session.currentFile.sourcePaths.map(p => fs.readFileSync(p));
          ...
          fs.writeFileSync(newPath, mergeResult.pdf);
          ...
          if (ctx.session.lastWizardMsgId) {
            await ctx.telegram.deleteMessage(ctx.chat.id, ctx.session.lastWizardMsgId).catch(() => { });
            ctx.session.lastWizardMsgId = null;
          }
        ...
        return ctx.reply(
          ...
          Markup.inlineKeyboard([
            [Markup.button.callback('✅ Продовжити зі старим', 'conflict_keep_old')],
            [Markup.button.callback('🆕 Розпочати з новим', 'conflict_start_new')]
          ])
        );
      }
      ...
      if (isPhoto) {
        ...
        try {
          await ctx.telegram.editMessageText(ctx.chat.id, ctx.session.lastMultiMsgId, null, doneText,
            Markup.inlineKeyboard([[Markup.button.callback('✅ Це все', 'multi_image_done')]])
          );
        } catch (e) {
          const statusMsg = await ctx.reply(doneText,
            Markup.inlineKeyboard([[Markup.button.callback('✅ Це все', 'multi_image_done')]])
          );
          ctx.session.lastMultiMsgId = statusMsg.message_id;
        }
      ...
      let finalMsg;
      if (result.preview) {
        finalMsg = await ctx.replyWithPhoto({ source: result.preview }, { caption: text, ...Markup.inlineKeyboard(buttons) });
      } else {
      ...
      const errorMsg = `❌ Помилка обробки: ${error.message}\n\n` +
      ...
      if (statusMsg) {
        try {
          await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, errorMsg, { parse_mode: 'Markdown', ...keyboard });
        } catch (e) {
  ...
  bot.action(/wizard_color_(.+)/, async (ctx) => {
    if (!ctx.session.printSettings) return ctx.reply('Помилка сесії. Почніть спочатку: /start');
    ctx.session.printSettings.color = ctx.match[1] === 'bw' ? 'Чорно-білий' : 'Кольоровий';
  ...
  bot.action(/wizard_pps_(\d)/, async (ctx) => {
    ...
    if (pps < cpp) {
      return ctx.answerCbQuery(`Помилка: Неможливо розмістити ${cpp} копій на ${pps} комірках!`, { show_alert: true });
    }
  ...
  async function generateAndSendCheckPdf(ctx) {
    ...
    try {
      ...
      const summary = `🏁 **Перевірка налаштувань**\n\n` +
        `📂 Тип: ${s.type}\n` +
        `🎨 Колір: ${s.color}\n` +
      ...
      await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, `❌ Помилка при генерації прев'ю: ${error.message}`, keyboard);
  ...
  async function showFinalSummary(ctx) {
    ...
    const summary = `✅ Налаштування завершено!\n\n` +
      `📄 Файл: ${f.name}\n` +
      `👥 Тип: ${s.type}\n` +
      `🎨 Колір: ${s.color}\n` +
    ...
    const isMockPreview = f.preview === Buffer.from('mock-preview-data').toString('base64') ||
    ...
    if (f.preview && !isMockPreview) {
      ...
      if (ctx.callbackQuery) {
        ...
        const res = await ctx.replyWithPhoto({ source: previewBuffer }, { caption: summary, ...keyboard });
        ctx.session.lastWizardMsgId = res.message_id;
      }
    } else {
      if (ctx.callbackQuery) {
  ...
  async function handlePrint(ctx) {
    ...
    try {
      ...
      if (result.success) {
        ...
        const feedback = `✅ Завдання відправлено на друк!\n\n` +
          `Статистика за місяць: ${totalPages} стор.`;

        const keyboard = Markup.inlineKeyboard([
          [Markup.button.callback('➕ Надрукувати наступне', 'action_print_next')]
        ]);
  ...
  bot.action('action_cancel_print', async (ctx) => {
    ...
    await ctx.editMessageText('❌ Друк скасовано. Надішліть новий файл для початку.',
      Markup.inlineKeyboard([[Markup.button.callback('🏠 Головне меню', 'type_selection_restart')]])
    );
  });
  ...
  bot.action('conflict_keep_old', async (ctx) => {
    ...
    await ctx.editMessageText(`✅ ОК, продовжуємо з файлом "${ctx.session.currentFile.name}".\nОстаннє повідомлення майстра актуальне.`);
  });

  bot.action('conflict_start_new', async (ctx) => {
    if (!ctx.session.pendingFile) return ctx.answerCbQuery('Помилка: файл не знайдено.');
    ...
    try {
      if (pending.isPhoto) {
        ...
        const fileBuffer = fs.readFileSync(pending.path);
        ...
        ctx.session.printSettings = { ...ctx.session.printSettings, copies: 1, color: 'Кольоровий', duplex: 'Ні' };

        const wizardText = `📄 Файл: ${pending.fileName}\n📏 Сторінок: ${result.basicParams.pages}\n\nОберіть наступну дію:`;
        const buttons = [
          [Markup.button.callback('🚀 Просто надрукуй це', 'action_print_direct')],
        ...
        let finalMsg;
        if (result.preview) {
          finalMsg = await ctx.replyWithPhoto({ source: result.preview }, { caption: wizardText, ...Markup.inlineKeyboard(buttons) });
        } else {
          finalMsg = await ctx.reply(wizardText, Markup.inlineKeyboard(buttons));
        }
        ctx.session.lastWizardMsgId = finalMsg.message_id;
      }
    } catch (err) {
  ...
  bot.action('multi_image_done', async (ctx) => {
    if (!ctx.session.multiImages || ctx.session.multiImages.length === 0) {
      return ctx.answerCbQuery('Ви не додали жодного зображення!', { show_alert: true });
    }
    ...
    try {
      const images = ctx.session.multiImages.map(img => fs.readFileSync(img.path));
      const mergeResult = await mergeImagesToPdf(images, false); // Завжди об'єднуємо в кольорі

      const fileName = `merged_${Date.now()}.pdf`;
      const filePath = getTempPath(fileName);
      fs.writeFileSync(filePath, mergeResult.pdf);
      ...
      let finalMsg;
      if (previewBuffer) {
        finalMsg = await ctx.replyWithPhoto({ source: previewBuffer }, { caption: text, ...Markup.inlineKeyboard(buttons) });
      } else {
        finalMsg = await ctx.reply(text, Markup.inlineKeyboard(buttons));
      }
      ctx.session.lastWizardMsgId = finalMsg.message_id;
      ...
      await ctx.reply('❌ Помилка при створенні PDF: ' + error.message, keyboard);
    }
  });
}

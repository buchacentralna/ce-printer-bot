...
export async function processPdf(fileBuffer, options = {}) {
  ...
  if (options.sourcePaths && options.sourcePaths.length > 0) {
    if (options.sourcePaths.length === 1) {
      ...
      const imgBuffer = await fs.readFile(options.sourcePaths[0]);
      pdfDoc = await createPdfFromImage(imgBuffer, grayscale);
    } else {
      // Багато оригіналів
      const imageBuffers = await Promise.all(
        options.sourcePaths.map(async (p) => await fs.readFile(p)),
      );
    ...
    ext === "pdf" ||
    currentBuffer.toString("ascii", 0, 4) === "%PDF"
  ) {
    pdfDoc = await PDFDocument.load(currentBuffer);
  } else if (
    ["jpg", "jpeg", "png", "webp", "tiff", "heic", "heif"].includes(ext)
  ) {
    isImage = true;
  ...
  if (pagesCount > 100) {
    throw new Error(
      `Занадто багато сторінок (${pagesCount}). Максимально дозволено 100.`,
    );
  }
...
async function grayscalePdfUsingGS(buffer) {
  ...
  try {
    ...
    if (await fileExistsAsync(tempOut)) {
    ...
    if (await fileExistsAsync(tempOut)) await fs.unlink(tempOut);
  }
}
 ...
 */
async function embedPageToGrid(
  srcDoc,
  dstDoc,
  dstPage,
  pageIndex,

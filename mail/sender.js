...
export async function applyOptionsToPdf(pdfBuffer, options) {
    ...
    const processed = await processPdf(currentBuffer, {
        grayscale: options.color === 'Чорно-білий',
    ...
    if (options.copies && options.copies > 1) {
        ...
        if (finalDoc.getPageCount() > 100) {
            throw new Error(`Результат перевищує 100 сторінок (${finalDoc.getPageCount()}). Зменште кількість копій або обсяг файлу.`);
        }
    }
...
function buildSubject(fileName, options) {
    ...
    if (options.color) parts.push(`Color: ${options.color === 'bw' ? 'B&W' : 'Color'}`);
